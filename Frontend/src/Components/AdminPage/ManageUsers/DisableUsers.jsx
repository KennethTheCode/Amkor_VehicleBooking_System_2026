...
function DisableUsers({ id, users, onStatusChanged }) {
    const [showModal, setShowModal] = useState(false)
    const [error, setError] = useState("")
    const [loading, setLoading] = useState(false)

    const isDisabled = users.status === "Disabled"
    ...
    const handleToggle = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}/ManageUsers/DisableUsers.php`, {
                ...
                body: JSON.stringify({ user_id: id, status: targetStatus }),
            });

            const data = await response.json();

            if (!data.success) {
                setError(data.message || `Failed to ${actionLabel.toLowerCase()} user.`);
                setLoading(false)
                return;
            }
                ...
                <div className="bg-black/20 fixed flex items-center justify-center h-screen inset-0 z-100">
                    <div className="bg-white rounded">
                        <div className="p-5 border-b-2 border-blue-800">
                            <p className="font-bold">Are you sure you want to {actionLabel.toLowerCase()} this user?</p>
                        </div>
